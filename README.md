<div align="center">

---

## 🙋 Sobre mí

Soy **Alejandro**, desarrollador web localizado en **Colombia/Villavicencio** con experiencia construyendo proyectos fullstack con el ecosistema **Node.js**. Me apasiona aprender cosas nuevas rápidamente y siempre estoy buscando mejorar mis habilidades.

Mi camino me ha llevado desde **HTML, CSS y JavaScript** hasta **Node.js + MySQL**, y hoy sigo expandiéndome en dos direcciones: consolidarme como desarrollador **fullstack** con **React + Vite** en el frontend y **Express 5 + Knex** en el backend, y explorar aplicaciones de escritorio con **Flet (Python)**.

Me defino como una persona **colaborativa, responsable y honesta**, que disfruta trabajar en equipo y aportar soluciones concretas a los problemas.

---

## 🛠️ Tecnologías y herramientas

### Frontend

![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Vite](https://img.shields.io/badge/Vite-646CFF?style=for-the-badge&logo=vite&logoColor=white)
![React Router](https://img.shields.io/badge/React_Router-CA4245?style=for-the-badge&logo=react-router&logoColor=white)
![Axios](https://img.shields.io/badge/Axios-5A29E4?style=for-the-badge&logo=axios&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)

### Backend

![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000?style=for-the-badge&logo=express&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white)
![Knex](https://img.shields.io/badge/Knex-D26B38?style=for-the-badge&logo=knex&logoColor=white)
![JWT](https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white)
![Bcrypt](https://img.shields.io/badge/Bcrypt-003A70?style=for-the-badge)
![Nodemon](https://img.shields.io/badge/Nodemon-76D04B?style=for-the-badge&logo=nodemon&logoColor=black)
![CORS](https://img.shields.io/badge/CORS-2ea44f?style=for-the-badge&logo=cors&logoColor=white)
![dotenv](https://img.shields.io/badge/dotenv-ECD53F?style=for-the-badge&logo=.env&logoColor=black)

### Escritorio — en aprendizaje

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Flet](https://img.shields.io/badge/Flet-000000?style=for-the-badge&logo=flutter&logoColor=white)

### Herramientas

![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)
![ESLint](https://img.shields.io/badge/ESLint-4B32C3?style=for-the-badge&logo=eslint&logoColor=white)
![Excel](https://img.shields.io/badge/Microsoft_Excel-217346?style=for-the-badge&logo=microsoft-excel&logoColor=white)

---

## 📂 Proyectos destacados

### ⚙️ [fullstack-learning](https://github.com/pulposky/fullstack-learning)

Proyecto personal de práctica para nivelar el trabajo fullstack: **React (JSX)** en el cliente y **Node.js/Express + Knex** en el servidor. Evoluciona progresivamente de JavaScript a **TypeScript**. Cubre React con componentes, props, estado y JSX; React Router para navegación; CSS Modules; API REST con Express; acceso a MySQL mediante Knex (query builder); migraciones y arquitectura próxima **MVC** con TypeScript (`src/`).

> Frontend: React 19 · Vite · React Router · CSS Modules · Axios
> Backend: Node.js · Express 5 · Knex · MySQL

### 🔐 [Apirest](https://github.com/pulposky/Apirest)

Sistema web de gestión de productos y usuarios con **autenticación, roles y CRUD completo** — Node.js + MySQL. Login seguro con sesiones, protección de rutas con middleware, roles **Admin/Usuario** (el usuario gestiona productos pero no usuarios), arquitectura **MVC** (controllers, models, middleware, routes, views EJS) y frontend público con HTML/CSS/JS.

### 🛒 [Sistema_POS](https://github.com/pulposky/Sistema_POS)

Sistema POS en **Python con Flet** para gestión de ventas, remisiones, clientes y productos. Inventario en tiempo real, registro de ventas y control de caja, y generación de **facturas PDF**. Incluye instalador configurado con **PyInstaller** y se conecta con el **Marketplace Web** compartiendo la misma base de datos `senapdv`.

### 🕹️ [Arcade2D](https://github.com/pulposky/Arcade2D)

Proyectos de videojuegos 2D enfocados en el aprendizaje, la práctica y la experimentación en desarrollo de juegos con Python.

### 🛍️ [Marketplace](https://github.com/pulposky/Marketplace)

Plataforma de comercio web en **JavaScript** (Node.js + Express + EJS + MySQL): catálogo público con búsqueda y filtros, sistema de apartados con notificaciones, dashboard de estadísticas y registro de visitas. Se integra con el Sistema POS vía base de datos compartida.

---

## 🌱 Lo que estoy aprendiendo ahora

- **Frontend:** React 19, Vite, React Router, Axios, CSS Modules
- **Backend:** Express 5, Knex (query builder), MySQL, JWT, Bcrypt
- **Migración:** JavaScript → **TypeScript** en frontend (`.tsx`) y backend (`.ts`)
- **Arquitectura:** MVC (controllers, models, middleware, routes)
- **Escritorio:** Python + Flet

---

## 🧠 Soft skills

- 🤝 **Colaborativo** — Trabajo bien en equipo y me adapto a diferentes dinámicas de trabajo
- ✅ **Responsable** — Cumplo con los compromisos y plazos establecidos
- 🤲 **Honesto** — Comunicación clara y transparente en todo momento
- ⚡ **Aprendizaje rápido** — Capacidad de asimilar nuevas tecnologías y conceptos con agilidad

---

## 📬 Contáctame

<div align="center">

[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:alejogarcia0324@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/pulposky)

</div>

---

<div align="center">

*"El mejor código que puedo escribir hoy es mejor que el que escribí ayer."*

⭐ Si alguno de mis proyectos te es útil, ¡no dudes en dejar una estrella!

</div>
